...
char str[55];
int dp[55][4][6], len, cs;
int vis[55][4][6];

int solve(int pos, int vwl, int cnsn)
{
    if(vwl==3 || cnsn==5) return 1;
    if(pos==len) return 2;
    if(vis[pos][vwl][cnsn]==cs) return dp[pos][vwl][cnsn];
    vis[pos][vwl][cnsn] = cs;
    int ret = 0 ;
    if(str[pos]=='?')
        ret = solve(pos+1, vwl+1, 0) | solve(pos+1, 0, cnsn+1);
    else if(isvowel(str[pos]))
        ret = solve(pos+1, vwl+1, 0);
    else
        ret = solve(pos+1, 0, cnsn+1);

    dp[pos][vwl][cnsn] = ret;
    return dp[pos][vwl][cnsn];
}
